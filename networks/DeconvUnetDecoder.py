...
class UnetDecoder(nn.Module):
    def __init__(self, encoder_hidden_sizes, n_classes=12):
        ...
        self.up_samples = nn.ModuleList([nn.ConvTranspose2d(self.hidden_sizes[i+1], self.hidden_sizes[i+1], 4, 2, 1) for i in range(len(self.hidden_sizes)-2)])# nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
